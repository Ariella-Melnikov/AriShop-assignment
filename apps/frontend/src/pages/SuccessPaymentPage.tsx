...
//   const products = useSelector((state: RootState) => state.products.products)
  ...
  const deliveryAddress = user?.addresses.find(a => a._id === user.defaultAddressId) || anonymousUser.deliveryAddress
  ...
  return (
    <div className='success-page'>
      ...
      <section className='order-summary'>
        ...
        <p><strong>Approval #:</strong> {createdOrder.payment.approvalNumber}</p>
        <p><strong>Delivery Date:</strong> {new Date(createdOrder.deliveryDate).toLocaleDateString()}</p>

        <h3>Shipping To</h3>
        <p>{createdOrder.deliveryAddress.street}, {createdOrder.deliveryAddress.city}</p>

        <h3>Billing To</h3>
        <p>{createdOrder.billingAddress.street}, {createdOrder.billingAddress.city}</p>

        <h3>Customer</h3>
        <p>{createdOrder.userInfo.firstName} {createdOrder.userInfo.lastName}</p>
